...
from mmdet.models.losses.expand_onehot_labels import _expand_onehot_labels
...
class MILLoss(nn.Module):
    ...
    def gfocal_loss_new(self, p, q, w=1.0):
        l1 = q*((1 - p) ** 2)
        l2=(1-q)*(p**2)
        l3 = l1 * (p + self.eps).log() + l2 * (1 - p + self.eps).log()
        return -(l3*w).sum(dim=-1)
    def neg_loss(self, p, q, w=1.0):
        l1 = p**2
        l2 = q * (p + self.eps).log()
        return -(l1 * l2 * w).sum(dim=-1)
    def forward(self, bag_cls_prob, bag_ins_outs, labels, valid, weight=None):
        ...
        labels = _expand_onehot_labels(labels, None, C)[0].float() # change by H
        # labels = _expand_onehot_labels(labels, None, C,None)[0].float() # change by H
        ...
        elif self.loss_type == 'gfocal_loss_new':
            loss = self.gfocal_loss_new(prob, labels, label_weights)
