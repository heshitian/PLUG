...
class P2BNet(TwoStageDetector): 
    ...
    def forward_train(self,
        ...
        fine_proposal_cfg = self.train_cfg.get('fine_proposal',
                                               self.test_cfg.rpn)
        losses = dict()
        gt_points = [bbox_xyxy_to_cxcywh(b)[:, :2] for b in gt_pseudo_bboxes]
        ...
        for stage in range(self.num_stages):
            if stage == 0:
                ...
                # Img_draw = Image.open(img_metas[0]['filename'])
                # Img_draw = Img_draw.resize((800,512))
                # draw = ImageDraw.Draw(Img_draw)
                # for rect_bbox in gt_bboxes[0]:
                #     draw.rectangle([int(rect_bbox[0]), int(rect_bbox[1]), int(rect_bbox[2]), \
                #         int(rect_bbox[3])], fill = None, outline = (0,255,0), width = 5)
                # Img_draw.show()
    ...
    def simple_test(self, img, img_metas, gt_bboxes, gt_anns_id, gt_pseudo_bboxes, gt_labels,
        ...
        x = self.extract_feat(img)
        for stage in range(self.num_stages):
            gt_points = [bbox_xyxy_to_cxcywh(b)[:, :2] for b in gt_bboxes[0]] # change by H gt_bboxes
            if stage == 0:
                generate_proposals, proposals_valid_list = gen_proposals_from_cfg(gt_points, base_proposal_cfg,
                                                                                  img_meta=img_metas)
            else:
            ...
            test_result, pseudo_boxes = self.roi_head.simple_test(stage,
                                                                  ...
                                                                  gt_pseudo_bboxes[0], gt_labels[0],
                                                                  gt_anns_id[0],
                                                                  img_metas,
                                                                  rescale=rescale)
        return test_result
