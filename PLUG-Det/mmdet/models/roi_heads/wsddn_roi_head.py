...
class WSDDNRoIHead(BaseRoIHead, BBoxTestMixin, MaskTestMixin):
    ...
    def _bbox_forward_train(self, x, proposal_list, gt_labels, img_metas):
        ...
        #rois = bbox2roi([res.bboxes for res in proposal_list])
        rois = bbox2roi(proposal_list).to(torch.float32)
        bbox_results = self._bbox_forward(x, rois)
        # gt_labels_loss = torch.cat(gt_labels).reshape(-1,15)
        # gt_labels_loss = gt_labels_loss[ rois[:,0].to(int)] 
        loss_bbox = self.bbox_head.loss(bbox_results['cls1'], bbox_results['cls2'], gt_labels)
    ...
    def simple_test_bboxes(self,
        ...
        rois = bbox2roi(proposals).to(torch.float32)
        bbox_results = self._bbox_forward(x, rois)
