...
class LoadAnnotations:
    ...
    def __init__(self,
        ...
        self.with_vertexes = with_vertexes
        # self.with_pseudo_bboxes = with_pseudo_bboxes
    ...
    def _load_bboxes(self, results):
        ...
        #change by H
        gt_pseudo_bboxes = ann_info.get('pseudo_bboxes', None) 
        if gt_pseudo_bboxes is None:
            gt_pseudo_bboxes = ann_info.get('pseudo_bboxes', None)
        if gt_pseudo_bboxes is not None:
            results['gt_pseudo_bboxes'] = gt_pseudo_bboxes.copy()
            results['bbox_fields'].append('gt_pseudo_bboxes')
    ...
    def _load_vertexes(self, results):
        """Private function to load mask annotations.
        ...
        gt_vertexes = results['ann_info']['vertexes']
        vertex_num = gt_vertexes.shape[0]
        img_shape = results['img_shape']
        # print(results['img_info']['file_name'])
        if vertex_num != 0:
            gt_vertexes[:, 0::2] = np.clip(gt_vertexes[:, 0::2], 0, img_shape[1]-1)
            gt_vertexes[:, 1::2] = np.clip(gt_vertexes[:, 1::2], 0, img_shape[0]-1)
        else:
            print('vertex_num is 0')
        results['gt_vertexes'] = gt_vertexes
        results['vertexes_fields'].append('gt_vertexes')
    ...
    def __call__(self, results):
        ...
        if self.with_vertexes:
            results = self._load_vertexes(results)
        return results
    ...
    def __repr__(self):
        ...
        repr_str += f'with_vertexes={self.with_vertexes}, '
...
class LoadWeakAnnotations(object):
    ...
    def _load_labels(self, results):
        ...
        labels_onehot = np.zeros((15, ), dtype=np.int32)
...
class LoadProposals(object):
    ...
    def __call__(self, results):
        ...
        proposals = results['proposals']['selective_search'] ### HHH
