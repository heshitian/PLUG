...
class Resize:
    ...
    def _resize_pseudo_bboxes(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        for key in results.get('pseudo_bbox_fields', []):
            bboxes = results[key] * results['scale_factor']
            if self.bbox_clip_border:
                img_shape = results['img_shape']
                bboxes[:, 0::2] = np.clip(bboxes[:, 0::2], 0, img_shape[1])
    ...
    def _resize_coarse_points(self, results):
        """Resize bounding boxes with ``results['scale_factor']``."""
        for key in results.get('coarse_points_fields', []):
            bboxes = results[key] * results['scale_factor']
            results[key] = bboxes
    ...
    def _resize_vertexes(self, results):
        """Resize masks with ``results['scale']``"""
        for key in results.get('vertexes_fields', []):
            bboxes = results[key] * results['scale_factor']
            results[key] = bboxes
    ...
    def __call__(self, results):
        ...
        self._resize_pseudo_bboxes(results)
        self._resize_coarse_points(results)
        self._resize_masks(results)
        self._resize_seg(results)
        self._resize_vertexes(results)
        return results
...
class RandomFlip:
    ...
    def bbox_flip(self, bboxes, img_shape, direction):
        ...
        return flipped
    def vertex_flip(self, vertexes, img_shape, direction):
        """Flip bboxes horizontally.
        ...
        assert vertexes.shape[-1] % 4 == 0
        flipped = vertexes.copy()
        if direction == 'horizontal':
            w = img_shape[1]
            flipped[..., 0::2] = w - vertexes[..., 0::2]
        elif direction == 'vertical':
            h = img_shape[0]
            flipped[..., 1::2] = h - vertexes[..., 1::2]
        elif direction == 'diagonal':
            w = img_shape[1]
            h = img_shape[0]
            flipped[..., 0::2] = w - vertexes[..., 0::2]
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            ...
            #flip vertexes
            for key in results.get('vertexes_fields', []):
                results[key] = self.vertex_flip(results[key],
                                              results['img_shape'],
                                              results['flip_direction'])
            # flip segs
            for key in results.get('seg_fields', []):
