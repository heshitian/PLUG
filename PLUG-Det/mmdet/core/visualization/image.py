...
def imshow_bboxes_points(img,
    ...
    assert pseudo_bboxes is None or pseudo_bboxes.shape[1] == 4, \
        f' pseudo_bboxes.shape[1] should be 4 , but its {pseudo_bboxes.shape[1]}.'
    ...
    if score_thr > 0:
        ...
        bboxes = bboxes[inds, :]
        pseudo_bboxes = pseudo_bboxes[inds, :]
        labels = labels[inds]
    ...
    if bboxes is not None:
        ...
        draw_bboxes(ax,pseudo_bboxes,colors, alpha=0.8, thickness=thickness)
        horizontal_alignment = 'left'
...
def imshow_det_bboxes_cp(img,
    ...
    text_palette = palette_val(get_palette(text_color, max_label + 1))
    text_colors = [text_palette[label] for label in labels]
    #change by H 将没有优化的点显示为红色
    scores = bboxes[:, -1]
    text_colors[scores==0] = (220, 20, 60)
